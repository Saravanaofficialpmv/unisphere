...
final studentResumeProvider = FutureProvider.family<StudentResumeModel?, String>((ref, identifier) async {
  final resumeService = ref.watch(resumeServiceProvider);
  return resumeService.generateResumeForStudent(identifier);
});
...
class ResumeService {
  ...
  Stream<StudentResumeModel?> watchResumeForStudent(String uidOrRegNo) async* {
    // 1. Immediately yield initial generated resume to ensure instant UI rendering
    try {
      final initial = await generateResumeForStudent(uidOrRegNo);
      if (initial != null) {
        yield initial;
      }
    } catch (e) {
    ...
    if (firestore != null && uidOrRegNo.isNotEmpty) {
      try {
        ...
        await for (final _ in stream) {
          final updated = await generateResumeForStudent(uidOrRegNo);
          if (updated != null) {
            yield updated;
          }
        }
  ...
  Future<StudentResumeModel?> generateResumeForStudent(String uidOrRegNo) async {
    ...
    if (firestore != null) {
      try {
        final doc = await firestore.collection('users').doc(cleanId).get().timeout(const Duration(milliseconds: 1500));
        if (doc.exists && doc.data() != null) {
          user = UserModel.fromMap(doc.data()!, doc.id);
        } else {
          final q = await firestore.collection('users').where('metadata.registerNumber', isEqualTo: cleanId).limit(1).get().timeout(const Duration(milliseconds: 1500));
    ...
    if (firestore != null) {
      try {
        final doc = await firestore.collection('students').doc(cleanId).get().timeout(const Duration(milliseconds: 1500));
        if (doc.exists && doc.data() != null) {
          student = StudentModel.fromMap(doc.data()!, doc.id);
        } else {
          final q = await firestore.collection('students').where('registerNumber', isEqualTo: cleanId).limit(1).get().timeout(const Duration(milliseconds: 1500));
          if (q.docs.isNotEmpty) {
            student = StudentModel.fromMap(q.docs.first.data(), q.docs.first.id);
          } else {
            final q2 = await firestore.collection('students').where('userId', isEqualTo: cleanId).limit(1).get().timeout(const Duration(milliseconds: 1500));
    ...
    if (firestore != null) {
      try {
        ...
        final doc = await firestore.collection('student_profiles').doc(regNo).get().timeout(const Duration(milliseconds: 1500));
        if (doc.exists && doc.data() != null) {
          profileMap = doc.data()!;
        } else {
          final doc2 = await firestore.collection('student_profiles').doc(user?.uid ?? cleanId).get().timeout(const Duration(milliseconds: 1500));
    ...
    if (firestore != null) {
      try {
        ...
        final snap = await firestore.collection('projects').get().timeout(const Duration(milliseconds: 1500));
        projects = snap.docs
    ...
    if (firestore != null) {
      try {
        ...
        final snap = await firestore.collection('certifications').get().timeout(const Duration(milliseconds: 1500));
        certs = snap.docs
    ...
    if (firestore != null) {
      try {
        ...
        final snap = await firestore.collection('hackathonRegistrations').get().timeout(const Duration(milliseconds: 1500));
        final snap2 = await firestore.collection('hackathon_registrations').get().timeout(const Duration(milliseconds: 1500));
    ...
    // Fallback extraction from metadata/profile for demo student if Firestore collections are unseeded
    ...
    final isDemoAccount = user?.email.toLowerCase() == 'saravanapmvofficial@gmail.com' ||
        cleanId == 'DEMO-STU' ||
        cleanId == 'RA2111003010001';

    // ── Build Header ──
    final fullName = (personalObj['fullName']?.toString().isNotEmpty == true)
        ? personalObj['fullName'].toString()
        : (user?.fullName.isNotEmpty == true
            ? user!.fullName
            : (student?.fullName.isNotEmpty == true ? student!.fullName : 'Alex Johnson'));
    ...
    final linkedinUrl = userMeta['linkedinUrl']?.toString().isNotEmpty == true
        ? userMeta['linkedinUrl'].toString()
        : (isDemoAccount ? 'https://www.linkedin.com/in/saravana-selvaraju/' : null);

    final githubUser = userMeta['githubUsername']?.toString().isNotEmpty == true
        ? userMeta['githubUsername'].toString()
        : (isDemoAccount ? 'Saravanaofficialpmv' : null);
    final githubUrl = githubUser != null && githubUser.isNotEmpty
    ...
    final leetcodeUser = userMeta['leetcodeUsername']?.toString().isNotEmpty == true
        ...
        : (isDemoAccount ? 'saravanapmv' : null);
    final leetcodeUrl = leetcodeUser != null && leetcodeUser.isNotEmpty
        ? 'https://leetcode.com/u/$leetcodeUser'
        : null;

    final portfolioUrl = userMeta['portfolioUrl']?.toString().isNotEmpty == true
        ? userMeta['portfolioUrl'].toString()
        : (githubUrl != null ? '$githubUrl?tab=repositories' : null);
    ...
    final cgpa = student?.cgpa ?? userMeta['cgpa']?.toString() ?? (isDemoAccount ? '8.92' : '8.50');
    ...
    if (isDemoAccount) {
      experienceList.add(const ResumeExperienceItem(
        id: 'exp-1',
    ...
    if (projects.isNotEmpty) {
      ...
      projectList.add(const ResumeProjectItem(
        id: 'proj-demo-1',
        ...
        technologies: ['Flutter', 'Firebase', 'Dart', 'Riverpod', 'Cloud Firestore', 'RBAC'],
      ...
      projectList.add(const ResumeProjectItem(
        ...
        technologies: ['Python', 'OpenCV', 'TensorFlow', 'Flutter', 'REST APIs'],
    ...
    if (isDemoAccount || allCollectedSkills.isEmpty) {
